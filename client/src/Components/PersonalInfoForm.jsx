...

            {
                typeof data.image === 'object' && (
                    <div className='flex flex-col gap-1 pl-4 text-sm'>
                        <p className=' font-bold mt-6 text-gray-500 hover:text-gray-900 cursor-pointer'>Remove Background</p>
            ...
            fields.map((field)=>{
                ...
                return (
                    <div className='space-y-1 mt-5' key={field.key}>
                        ...
                        <input type={field.type} value={data[field.key] || ""} onChange={(e)=>handleChange(field.key,e.target.value)} className='mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focused:ring focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors text-sm' placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required}/>
                    </div>
                )
            })
        }
